...
def adjust_light(image, scale):
    background = np.ones_like(image) * 255
    if scale >= 0:
        out = image * (1-scale) + background * scale
    else:
        out = image * (scale+1)
...
for iteration in tqdm.tqdm(range(num_iterations)):
    ...
    dimension =  dimension.unsqueeze(1).repeat(1,light_level)
    repo = repo[dimension].view(-1,light_level)
...
plt.xlabel('Lighting adjustment')
