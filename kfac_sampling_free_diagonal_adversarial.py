...
for iteration in tqdm.tqdm(range(num_iterations)):
    ...
    # '/root/Documents/BNN_KFAC/data/kitti/training/label_2/000206.txt'
    ...
    net.phase = 'train'
    out = net(xx)
    target = [torch.tensor(t,dtype=torch.float32) for t in target]
    # target = [target[3]]
    ...
    image_grad = xx.grad
    print('IMG  ','avg:',xx.mean(),'max:',xx.max())
    print('GRAD ','avg:',image_grad.mean(),'max:',image_grad.max())

    xx = xx + torch.sign(image_grad) * 0.015 * 255
    xx = xx.contiguous()
