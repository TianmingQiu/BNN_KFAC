...
def eval_unvertainty_diag(model, x, H, diag):
    ...
    for i in range(detections.size(1)):
        for j in range(detections.size(2) - 1):
            if detections[0,i,j,0] >= threshold:
                ...
                minor_softmax = torch.cat((minor_softmax,detections[0,:i,j,0],detections[0,i+1:,j,0]))
    ...
    minor_softmax = minor_softmax[minor_softmax < 0.5]
    uncertainties = []
    ...
    minor_uncertainties = []
    for softmax_val in minor_softmax:
        softmax_val.backward(retain_graph = True)
        all_grad = torch.Tensor()
        for layer in model.modules():
            if layer.__class__.__name__ in diag.layer_types:
                if layer.__class__.__name__ in ['Linear', 'Conv2d']:
        ...
        pred_std = torch.abs(J * H * J).sum()
        del J, all_grad
        minor_uncertainties.append(pred_std)
    return out[1:], uncertainties, minor_uncertainties, minor_softmax
...
for iteration in tqdm.tqdm(range(num_iterations)):
    ...
    mean_predictions, uncertainty, minor_uncertainty, minor_softmax = eval_unvertainty_diag(net, xx, H, diag)
    ...
    minor_uncertainty = (torch.stack(minor_uncertainty) if len(minor_uncertainty) else torch.tensor([])) ** 0.5
    scale = torch.Tensor(rgb_image.shape[1::-1]).repeat(2)

    minor_uncertainty = minor_uncertainty.cpu()
    flag = 0
    for prediction,unc in zip(mean_predictions,uncertainty):
        y = minor_uncertainty[flag:flag+8]
        plt.scatter(unc[0] * torch.ones_like(y),y,c='k',alpha=0.1)
        plt.scatter(unc[0],y.mean(),c='goldenrod',alpha=1,marker='D')
        flag += 8
